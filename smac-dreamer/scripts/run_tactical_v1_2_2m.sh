...
: "${SOURCE_CHECKPOINT:?Set SOURCE_CHECKPOINT to a compatible non-tactical checkpoint}"
: "${JEPA_CHECKPOINT:?Set JEPA_CHECKPOINT to a compatible Exp-40 checkpoint}"
: "${RUN_DIR:?Set RUN_DIR to a fresh Tactical-v1.2 output directory}"
...
[[ -s "$REPO/$CONFIG" || -s "$CONFIG" ]] || { echo "[FAIL] config missing: $CONFIG" >&2; exit 1; }
...
fresh_phase_start_step=0
fresh_phase_final_step=$FINAL_STEP
EOF
